...
{   
   ...
   cout<<"Enter the a number from 2-32768 to decide the probability of IO occurance\nLOWER NUMBER INDICATES HIGHER IO OCCURANCE :";
   short prob=0;
   cin>>prob;
        ...
        if(pr.currentProcess->IO==false)
        {
            if(rand()%prob==0)
            {
              ...
              while(  (sleepTime >= pr.currentProcess->burst));
               ...
               cout<<"Executed time before IO :"<<sleepTime<<endl;
